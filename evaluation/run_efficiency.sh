...
python -m lmms_eval \
  --model ptd_qwen3_vl \
  --model_args "${COMMON},decoding=ptd,attn_implementation=sdpa,vision_attn_implementation=flash_attention_2,pbd_attn_implementation=flash_attention_2,efficiency_log=${OUTPUT_DIR}/ptd.jsonl" \
  --tasks ptd_vidstg \
  --batch_size 1 \
